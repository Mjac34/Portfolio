...
def process_loan_balances():
    df = pd.read_csv("Raw_Loan_Balance.csv", delimiter=";", dtype_backend="numpy_nullable")
...
def process_loan_transactions():
    df = pd.read_csv("Raw_Loan_Transaction.csv", delimiter=";", dtype_backend="numpy_nullable")
...
def run_pipeline():
    try:
        ...
        dim_LoanAccount.to_csv("Dim_LoanAccount.csv", index=False)
