...
try:
    ...
    from agents import llm_client  # noqa: F401 — when run as scripts from project root
...
class CRMCustomerProfileAgent(Agent):
    ...
    def _decide_next_best_action(segment: str, order_count: int, avg_order_value: float, recency_days: float) -> str:
        ...
        if segment_name == "High Value":
            if recency_days > 120:
                return "Retention – VIP reactivation"
            if avg_order_value > 5000:
                return "Upsell – premium assortment"
            return "Upsell – cross-sell to high-value products"
        if segment_name == "Medium Value":
            if recency_days > 90:
                return "Retention – reactivation campaign"
            if order_count >= 3:
                return "Campaign – targeted next-purchase offer"
            return "Campaign – introductory offer"
        if recency_days > 150:
            return "Retention – win-back campaign"
        if order_count <= 2:
            return "Campaign – new customer experience"
        return "Retention – recurring reactivation"
    ...
    def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for customer_id, profile in profiles.items():
            ...
            behavior_signal = "Active" if recency_days <= 90 else "At Risk" if recency_days <= 180 else "Inactive"
            ...
            churn_risk = 0
            if behavior_signal == "Active":
                churn_risk = max(0, min(30, int(30 - (recency_days * 0.2))))
            elif behavior_signal == "At Risk":
        ...
        for profile in final_profiles:
            ...
            if profile["behavior_signal"] == "Active" and profile["segment"] == "High Value":
                profile["status_label"] = "Active"
            elif profile["behavior_signal"] == "At Risk":
                profile["status_label"] = "At Risk"
            else:
...
class LLMInsightAgent(Agent):
    """Writes the narrative layer via an OpenAI-compatible LLM provider.

    Numbers are always computed deterministically upstream; the LLM only
    interprets them. Falls back to a template summary when no token is set.
    """

    def __init__(self):
        super().__init__("LLMInsightAgent")

    def _build_context(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        insights = input_data.get("insights") or {}
        profiles = input_data.get("customer_profiles") or []
        segments: Dict[str, int] = {}
        churn: Dict[str, int] = {}
        for p in profiles:
            segments[p.get("rfm_segment", "Unknown")] = segments.get(p.get("rfm_segment", "Unknown"), 0) + 1
            churn[p.get("churn_risk_label", "Unknown")] = churn.get(p.get("churn_risk_label", "Unknown"), 0) + 1
        return {
            "score_summary": input_data.get("summary") or {},
            "top_products": insights.get("top_products", []),
            "top_customers": insights.get("top_customers", []),
            "top_countries": insights.get("top_countries", []),
    ...
    def _fallback_narrative(self, ctx: Dict[str, Any]) -> str:
        lines = [
            "*(Generated without LLM — set LLM_API_KEY in .env for AI-written narrative)*",
            "",
            f"- {ctx['customer_count']} customer profiles built",
            f"- RFM segments: {', '.join(f'{k}: {v}' for k, v in sorted(ctx['rfm_segments'].items())) or 'n/a'}",
            f"- Churn risk: {', '.join(f'{k}: {v}' for k, v in sorted(ctx['churn_risk_distribution'].items())) or 'n/a'}",
        ]
    ...
    def run(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if not llm_client.is_configured():
            logger.info(f"{self.name}: LLM_API_KEY not set — using fallback narrative")
            narrative = self._fallback_narrative(ctx)
        else:
            messages = [
                {"role": "system", "content": (
            ...
            try:
                narrative = llm_client.chat(messages).content or ""
                logger.info(f"{self.name}: LLM narrative generated ({len(narrative)} chars)")
            except Exception:
                logger.exception(f"{self.name}: LLM call failed — using fallback")
                narrative = self._fallback_narrative(ctx)
        return {
            "rows": input_data.get("rows", []),
            "summary": input_data.get("summary"),
